...
def get_from_pages(url, params = {}, headers = {}):
    ...
    while len(next_page) == 100:
        page += 1
        next_page = session.get(url, params=merge_dictionaries(params, {'page': page}), headers=headers).json()
        yield next_page
...
def search(url, keyword, start_date, end_date):
    ...
    while True:
        params = {'q': f'\"{keyword}\"+created:{start_date}..{end_date}', 'per_page' : 100}
        ...
        for page in range(2, min([10 + 1, num_pages])):
            next_page = session.get(url, params=merge_dictionaries(params, {'page': page}), headers=api_header).json()
            yield next_page

...
def is_development_active(repository_url: str):
    ...
    authors = {}
    commit_count = 0
    for page in get_from_pages(
        repository_url + "/commits",
        {"since": enforcable_gdpr, "until": end_date, "per_page": 100},
    ):
        commit_count += len(page)
        for commit in page:
            authors[commit["commit"]["author"]["name"]] = True
            if len(authors.keys()) >= min_contributors and commit_count >= min_commits:
                return True
...
def is_issue_active(repository_url: str):
    ...
    for page in get_from_pages(
        ...
        {"since": enforcable_gdpr, "until": end_date, "per_page": min_issues},
    ):
